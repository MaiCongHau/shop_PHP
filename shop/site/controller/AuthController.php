...
class AuthController {
	// https://shop.com/site/index.php?c=auth&a=loginGoogle
	function loginGoogle() {
		try {
	        ...
	        if (isset($_GET['code'])) {
				...
				// mình lấy được 2 thông tin $email và $password 
			  	$email =  $google_account_info->email;
			  	$name =  $google_account_info->name;
			  	$this->createCustomerBySocial($email, $name, "google"); // tạo và lưu tài khoản 
			  	$this->setupLoginEnv($email, $name); // set cái session cho đăng nhập
	...
	function createCustomerBySocial($email, $name, $type) {
		...
		if (empty($customer)) { // nếu lần đầu tiên người dùng vào thì làm dell gì mà có trong database nên phải tạo lại
			//create new customer
			$data = array(
				"name" => $name,
				"mobile" => "",
				...
				"login_by" => $type,
				"is_active" => 1 // lưu ý nếu có trường is_active nhớ bật lên
			);
			$customerRepository->save($data); // lưu
		}
	}

	function setupLoginEnv($email, $name, $remember_me = null) {
		$_SESSION["email"] = $email; // set SESSION cho nó login luon 
		$_SESSION["name"] = $name;
	}
